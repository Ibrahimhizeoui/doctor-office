...
//		$champ = mb_convert_case(strip_tags($champ), MB_CASE_LOWER, $mb_encode);
	...
	/*---------------------------------------------------------------------------------*/
	function moteurAffichage($callback = '', $colonnesSelect = '', $limit = array(false, 0, 10, false), $ordre = array(false, "id", "DESC"), $algo = array(false,'algo','DESC','id'), $orderLimitPerso = '') {
		...
		if(empty($orderLimitPerso)) {
			//$this->requeteTotale = mysql_query("SELECT search.$selectColumn, * FROM search WHERE titre LIKE '%ti%' JOIN searchdeux WHERE searchdeux.titredeux LIKE '%ti%'")
	...
	/*----------------------------------------------------------------------------------------------------*/
	function moteurPagination($instruction = 0, $param = "", $NbVisible = 2, $debutFin = 0, $suivPrec = true, $firstLast = true, $arrayAff = array('&laquo; Précédent', 'Suivant &raquo;', 'Première page', 'Dernière page', 'precsuiv', 'current', 'pagination', 'inactif'), $arraySeparateur = array('&hellip;', ' ', ' ', ' ', ' ')) {
...
class affichageResultats extends moteurRecherche {
	function nbResultats($illimite = false, $wordsResults = array("résultat", "résultats"), $phrase = 'pour votre recherche', $coord = " à ") {
		if($illimite == true) {
			...
			if($this->limitArg == 0) {
				$nbDebut = 1;
				if($this->nbResults() > $this->limit) {
					$nbFin = ($this->limitArg+1) * $this->limit;
				} else {
					$nbFin = $this->nbResults();
				}
			} else {
				$nbDebut = (($this->limitArg-1) * $this->limit)+1;
				
				if(ceil($this->nbResults()/($this->limit*$this->limitArg)) != 1) {
					$nbFin = $this->limitArg * $this->limit;
				} else {
...
class autoCompletion {
	...
	function autoCompletion($urlDestination = "autocompletion.php", $selector = "#moteur", $tableName = "autosuggest", $colName = "words", $multiple = true, $limitDisplay = 5, $type = 0, $autoFocus = false, $create = true, $encode = "utf-8") {
	...
	function autoComplete($field = '', $minLength = 2) {
		$table = $this->table;
		$column = $this->column;
