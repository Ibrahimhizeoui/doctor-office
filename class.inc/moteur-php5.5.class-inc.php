	...
	public function moteurAffichage($callback = '', $colonnesSelect = '', $limit = array(false, 0, 10, false), $ordre = array(false, "id", "DESC"), $algo = array(false,'algo','DESC','id'), $orderLimitPerso = '') {
		...
		if(empty($orderLimitPerso)) {
			...
			if($limit[0] == true && $ordre[0] == true) {
				$this->requeteTotale = $this->db->query("SELECT $selectColumn FROM $this->tableBDD WHERE $this->condition $orderLimitPerso $this->orderBy $this->limitMinMax")
				or die("<div>Erreur dans la requête, vérifiez bien votre paramétrage complet !</div>");
			} else if($limit[0] == true && $ordre[0] == false) {
				$this->requeteTotale = $this->db->query("SELECT $selectColumn FROM $this->tableBDD WHERE $this->condition $orderLimitPerso $this->limitMinMax")
				or die("<div>Erreur dans la requête, vérifiez bien votre paramétrage complet !</div>");
			} else {
				$this->requeteTotale = $this->db->query("SELECT $selectColumn FROM $this->tableBDD WHERE $this->condition $orderLimitPerso")
				or die("<div>Erreur dans la requête, vérifiez bien votre paramétrage complet !</div>");
			}
			// Pour calculer le nombre total de résultats justes
